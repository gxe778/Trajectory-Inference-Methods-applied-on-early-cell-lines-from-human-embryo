...
TID <- read_excel("TI methods/TI Data.xlsx")
 
which(is.na(TIData), arr.ind=TRUE)

TID<-na.omit(TID)

write_excel_csv2(TID, 'TID.csv')

TIDD<-TID[,-2:-4]
TIDD<-as.data.frame(TIDD,row.names=TID$GeneID)
TIDD<-TIDD[,-1]



#TIDD<-as.data.frame.data.frame(TID, row.names=TID$GeneID)


cols<-as.factor(unique(TIDD$Cell))
rows<-TIDD$GeneID

a<-subset(TIDD,Cell=="oocyte1")
b<-subset(TIDD,Cell=="zygote1")
c<-subset(TIDD,Cell=="2 cell")
d<-subset(TIDD,Cell=="4 cell")
e<-subset(TIDD,Cell=="8 cell")
f<-subset(TIDD,Cell=="M1")
g<-subset(TIDD,Cell=="B1")
