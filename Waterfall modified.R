...
sce <- SingleCellExperiment::SingleCellExperiment(assay = list(counts = counts, logcounts=v))
...
# We chose desired number of groups k=5 to check the available cutoff branches or outliers
...
all[which(branch==2)] <-"#13751B" #2 darkgreen
all[which(branch==3)] <-"#F26A6A" #3 salmon
all[which(branch==4)] <-"#FF6A00" #4 orange
all[which(branch==5)] <-"#3AC4D6" #
all[which(branch==6)] <-"##0000FF" #4 orange
all[which(branch==7)] <-"#FF0000" #5 lightblue#5 lightblue
...
gene_name=gene_name
plot(pca18$x[,1:90],cex=scale(as.numeric(all[grep(gene_name,rownames(all)),match(rownames(all),colnames(all))]))*8+.3,pch=19,main=gene_name)

# 4. O:Z
gene_name=as.matrix(all)
plot(pca18$x[,1:2],col=paste0(all,99),cex=scale(as.numeric(all[grep(gene_name,rownames(all)),match(rownames(pca18$x),colnames(all))]))*8+.3,pch=19,main=gene_name)
